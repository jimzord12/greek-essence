...
- Observation: controller PID `44988` disappeared before its Python cleanup completed, while its owned Hermes root PID `33596` and descendants remained alive. The stale runtime lock still named the dead controller, `controller-state.json` still named the live root, and the tracked wrapper reported exit `0` despite no task progress. The exact external termination cause was not proven. The same failure reproduced on the next managed launch: controller PID `43772` disappeared while root PID `10172` survived, again with a stale lock/state and a false normal-exit notification.
